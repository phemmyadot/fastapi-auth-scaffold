...
- [ ] **8.8** Seed default roles: super_admin, admin, user [Spec §6 Role/UserRole]
- [ ] **8.9** `require_role("admin")` dependency — raises 403 if user lacks role [Spec §4 Complex]
- [ ] **8.10** `require_permission("users:delete")` dependency [Spec §4 Complex]
- [ ] **8.11** `GET /admin/users` — list all users, paginated (admin only) [Spec §7 Admin]
- [ ] **8.12** `POST /admin/users/{user_id}/roles` — assign role (super_admin only) [Spec §7 Admin]
- [ ] **8.13** `DELETE /admin/users/{user_id}/roles/{role_id}` — remove role (super_admin only) [Spec §7 Admin]
- [ ] **8.14** `POST /admin/users/{user_id}/ban` — set is_active=false, revoke all tokens [Spec §7 Admin]
- [ ] **8.15** `POST /admin/users/{user_id}/unban` — reactivate account [Spec §7 Admin]
- [ ] **8.16** Create `app/scripts/seed_roles.py` for `make seed` [Spec §11]

### OAuth2 (if ENABLE_OAUTH2=true) [Spec §4 Complex, §7 OAuth2]
- [ ] **8.17** Create `app/core/oauth2.py` — OAuth2 flows for Google + GitHub with PKCE, encrypted token storage [Spec §4 Complex, §8]
- [ ] **8.18** Wire `OAuthAccount` model from Phase 2.7 into routes [Spec §6 OAuthAccount]
- [ ] **8.19** `GET /auth/oauth/{provider}` — redirects to provider's OAuth consent screen with PKCE state [Spec §7]
- [ ] **8.20** `GET /auth/oauth/{provider}/callback` — handles callback, exchanges code for user info, creates or links user account [Spec §7]
- [ ] **8.21** Account linking: if email exists link to existing user; if not create new user (hashed_password=null) [Spec §4 Complex]
- [ ] **8.22** `GET /users/me/connected-accounts` — list linked OAuth providers [Spec §7]
- [ ] **8.23** `DELETE /users/me/connected-accounts/{provider}` — unlink guard: cannot remove last login method (must have password set or another provider linked) [Spec §4 Complex]
